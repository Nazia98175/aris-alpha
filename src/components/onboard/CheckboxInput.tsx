...
const CheckboxInput: React.FC<CheckboxInputProps> = ({ obj }) => {
    return (
        <label
            htmlFor={obj.id}
        ...
        >
            <div className="flex w-full items-center gap-3">
                ...
                <div className="w-full space-y-0.5 sm:space-y-1">
                    ...
                    {obj.desc && (
                        <p className="text-xs leading-[150%] text-[#3F3F3F] sm:text-sm sm:text-nowrap">{obj.desc}</p>
                    )}
                </div>
            </div>
            <div className="relative">
                <input type="checkbox" className="peer sr-only" id={obj.id} value={obj.value} />
